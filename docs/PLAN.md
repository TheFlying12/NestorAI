...
- Development runtime is temporarily set to `google/gemini-3-pro-preview` to unblock WhatsApp and cloud control-plane work.
- Local Ollama remains the target runtime for MVP hardening and Pi deployment phases.
- Phase 3 kickoff is in progress: gateway now includes WhatsApp webhook verify/inbound/outbound adapter scaffolding behind the shared provider boundary.
